...
pub fn test_all_precomputation_generation() {
    ...
    let (sk, sk_shares, a_shares, ask_terms) = generate_pcf_pcg_output(seed_pre, t, n, k);

    let per_party_precomputation = create_pp_precomputation_from_all_v_ole_evaluations(
        k, n, &sk_shares, &a_shares, &ask_terms,
    );

    test_pcf_pcg_output_ask(k, &indices, sk, &a_shares, &ask_terms);
...
pub fn test_pcf_pcg_output_ask(
    k: usize,

...
pub fn generate_pp_precomputation(
    ...
    let (sk, sk_shares, a_shares, ask_terms) = generate_pcf_pcg_output(seed_array, t, n, k);
    (
        sk,
...
pub fn generate_pcf_pcg_output(
    ...
    n: usize,
    k: usize,
) -> (Fr, Vec<Fr>, Vec<Vec<Fr>>, Vec<Vec<Vec<(Fr, Fr)>>>) {
    ...
    return (sk, sk_shares, a_shares, ask_terms);
}

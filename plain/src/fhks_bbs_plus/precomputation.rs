...
pub struct PerPartyPreSignature {
    ...
    pub s_share: Fr,           //s^k_i for k in [t]
    pub ae_term_own: Fr,       //a^k_i * e^k_i for k in [t]            //Might not be necessary
    pub as_term_own: Fr,       //a^k_i * s^k_i for k in [t]            //Might not be necessary
    ...
    pub as_terms_a: Vec<Fr>, //share of a^k_i * s^k_j for k in [t], j in [n] (j can also be i)
    pub as_terms_s: Vec<Fr>, //share of a^k_j * s^k_i for k in [t], j in [n] (j can also be i -- this time other share)
...
impl LivePreSignature {
    ...
    pub fn from_presignature_with_coefficients(
        ...
        let mut alpha_share = pre_signature.as_term_own;
        ...
        for (ind_j, &el_j) in indices.iter().enumerate() {
            if el_j != own_index {
                ...
                alpha_share.add_assign(&pre_signature.as_terms_a[el_j - 1]);
                alpha_share.add_assign(&pre_signature.as_terms_s[el_j - 1]);

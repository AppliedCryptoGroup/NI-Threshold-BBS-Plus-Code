...
pub fn test_all_precomputation_generation() {
    ...
    let (sk, sk_shares, a_shares, s_shares, as_terms, ask_terms) =
        generate_pcf_pcg_output(seed_pre, t, n, k);

    let per_party_precomputation = create_pp_precomputation_from_all_v_ole_evaluations(
        k, n, &sk_shares, &a_shares, &s_shares, &as_terms, &ask_terms,
    );

    test_pcf_pcg_output_ae_as_ask(
        k, &indices, sk, &a_shares, &s_shares, &as_terms, &ask_terms,
    );
...
pub fn test_pcf_pcg_output_ae_as_ask(
    ...
    for i_k in 0..k {
        ...
        let mut as_indirect = Fr::zero();
        let mut ask_indirect = Fr::zero();

        for i_n in &indices[i_k] {
            a.add_assign(&a_shares[i_k][i_n - 1]);
            s.add_assign(&s_shares[i_k][i_n - 1]);
        }

        as_direct.mul_assign(&a);
        as_direct.mul_assign(&s);
        ...
        for i_n in &indices[i_k] {
            for j_n in &indices[i_k] {
                let mut tmp_as = Fr::zero();
                let mut tmp_ask = Fr::zero();

                tmp_as.add_assign(&as_terms[i_k][i_n - 1][j_n - 1].0);
                tmp_as.add_assign(&as_terms[i_k][i_n - 1][j_n - 1].1);
                ...
                as_indirect.add_assign(&tmp_as);
                ask_indirect.add_assign(&tmp_ask);
            }
        }

        assert_eq!(
            as_direct, as_indirect,
            "Computation of AS is not consistent"
        );
        assert_eq!(
...
pub fn test_per_party_precomputations_with_coefficients(
    ...
    for i_k in 0..k {
        ...
        let mut s_direct = Fr::zero();
        let mut a_indirect = Fr::zero();
        let mut s_indirect = Fr::zero();
        ...
        for &el_i in &indices[i_k] {
            ...
            s_direct.add_assign(&s_shares[i_k][el_i - 1]);

            a_indirect.add_assign(&precomputations[el_i - 1].pre_signatures[i_k].a_share);
            s_indirect.add_assign(&precomputations[el_i - 1].pre_signatures[i_k].s_share);
        }

        let mut as_direct = a_direct;
        as_direct.mul_assign(&s_direct);

        let mut ask_direct = a_direct;
        ask_direct.mul_assign(&sk);

        for (ind_i, &el_i) in indices[i_k].iter().enumerate() {
            let mut share_of_as = precomputations[el_i - 1].pre_signatures[i_k].as_term_own;

            let mut share_of_ask = Fr::zero();
            let mut tmp_ask_own_lagrange =
                precomputations[el_i - 1].pre_signatures[i_k].ask_term_own;

            for (ind_j, &el_j) in indices[i_k].iter().enumerate() {
                if el_j != el_i {
                    share_of_as.add_assign(
                        &precomputations[el_i - 1].pre_signatures[i_k].as_terms_a[el_j - 1],
                    );
                    share_of_as.add_assign(
                        &precomputations[el_i - 1].pre_signatures[i_k].as_terms_s[el_j - 1],
                    );
